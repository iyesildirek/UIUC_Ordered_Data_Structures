...
static void treeFactory(GenericTree<int>& tree) {
  ...
  auto nodeE = nodeB->addChild(23);
  
  // Set height 3
  auto nodeF = nodeD->addChild(42);
...
std::vector<T> traverseLevels(GenericTree<T>& tree) {
  ...
  // Queue of node pointers that we need to explore
  std::queue<TreeNode*> nodesToExplore;

  // Push root node to queue
  nodesToExplore.push(rootNodePtr);
  
  // First pass loop flag
  int firstTime = 0;
  
  // Loop while there are still nodes to explore
  while (!nodesToExplore.empty()) {

    // Make a copy of the front pointer on the queue, 
	// then pop it to decrease the queue.
	// Only add front node during first pass of loop.
	...
	// Avoid adding a nullptr to queue.
    if (!frontNode) 
	{
      throw std::runtime_error("Error: Queued a null pointer");
    }

    // Empty vector of children node pointers 
    ...
    for (auto childPtr : frontNode->childrenPtrs) 
	{
      if (childPtr) 
	  {
        // Push childPtr to the back of traverse vector
        traverseChildrenPtrs.push_back(childPtr);
		
        // Add child pointer to the end of the exploration queue.
        nodesToExplore.push(childPtr);
		
		// Add current node explored data to results
